...
V3-Lite is provider-agnostic. It does not run a search engine in replay mode, does not call the OpenAI API, and does not require `OPENAI_API_KEY`. OpenAI Deep Research API support remains a future-ready provider scaffold, but current case runs can use OpenClaw / GPT-5.5 or human-assisted research as the external Deep Research executor.
