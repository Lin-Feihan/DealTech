...
def load_trace(example_dir: Path) -> dict:
...
def certify_example(example_dir: Path) -> dict:
    trace = example_dir / "trace"
    pce = example_dir / "pce_audit"
    pce.mkdir(parents=True, exist_ok=True)
    loaded = load_trace(example_dir)
    ...
    external_blocked = [r for r in pce_rows if r.get("delivery_scope") == "external_final" and r.get("certification_status") in BLOCKING | {INTERNAL}]
    ...
    delivery_gate = validate_final_delivery_claims(example_dir, pce_rows, claim_rows)
    ...
    allowed = final_delivery_allowed(overall, blockers) and delivery_gate["allowed"]
    result = {
