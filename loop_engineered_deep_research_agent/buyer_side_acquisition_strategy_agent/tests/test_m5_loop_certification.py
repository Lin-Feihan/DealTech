...
from loop_engineered_deep_research_agent.buyer_side_acquisition_strategy_agent.runtime.claim_certifier import validate_certification_result
from loop_engineered_deep_research_agent.buyer_side_acquisition_strategy_agent.runtime.repair_plan_builder import validate_repair_plan, validate_research_gaps
from loop_engineered_deep_research_agent.buyer_side_acquisition_strategy_agent.runtime.source_refetch_check import run_source_refetch_check
...
from loop_engineered_deep_research_agent.buyer_side_acquisition_strategy_agent.runtime.run_m5 import M5FailClosed, run_m5_pipeline


RUNTIME_ROOT = Path(__file__).resolve().parents[1]


class BuyerSideAcquisitionStrategyAgentM5LoopCertificationTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_sec_companyfacts_provider_is_disabled_by_default(self) -> None:
        ...
        with patch.dict(os.environ, {"ENABLE_SEC_COMPANYFACTS_PROVIDER": "", "SEC_USER_AGENT": ""}), patch(
            "loop_engineered_deep_research_agent.buyer_side_acquisition_strategy_agent.runtime.claim_certifier.make_sec_companyfacts_provider",
    ...
    def _run_certification_with_refetch_result(self, label: str, result_fields: dict) -> dict:
        ...
        with patch("loop_engineered_deep_research_agent.buyer_side_acquisition_strategy_agent.runtime.claim_certifier.run_source_refetch_check", return_value=[refetch_result]):
    ...
    def _run_certification_with_xbrl_result(self, label: str, result_fields: dict) -> dict:
        ...
        with patch("loop_engineered_deep_research_agent.buyer_side_acquisition_strategy_agent.runtime.claim_certifier.run_xbrl_numeric_check", return_value=[xbrl_result]):
            return self._run_certification_for_graph(graph, label)
